...
- **元素**：方向图标（12 向，含前方向左/右、左右行向后）、数字线路（多线路、色条自动配色）、文本线路（贯穿色条 + 大字 + 线/英文名，可关闭「线路名下沉」改与「线」同字号完整显示）、双语文本（中英双语三种对齐）、大文本（任意文本的大号展示，旧名「大数字」）、出入口、出口（出口黄底）、服务设施图标（电梯/洗手间/楼梯等 17 种）、空白占位。
